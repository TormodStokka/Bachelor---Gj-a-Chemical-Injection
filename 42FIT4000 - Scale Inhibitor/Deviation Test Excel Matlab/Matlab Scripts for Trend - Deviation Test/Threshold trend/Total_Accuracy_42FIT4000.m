...
% Accuracy value
TotalAccuracy = 110.22;

% Create figure
figure;

% Plot the total accuracy line
yline(TotalAccuracy, '-b', 'LineWidth', 2, 'DisplayName', '42FIT4000');
...
title('42FIT4000 in Comparison to External Flow Meter');
ylabel('Percentage', 'FontSize', 12, 'FontWeight', 'bold');
xlim([0, 1]); % No actual x-axis data, so we limit to one point
ylim([0, 150]); % Set y-axis limits from 0 to 110% to include threshold lines
grid on;
